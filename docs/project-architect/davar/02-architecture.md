...
4. **zustand + AsyncStorage as the persistence spine; SQLite left dormant.** `syncService` still imports `getDatabase`, so full SQLite removal is blocked until sync is refactored. `[current]`

...
| S-08 | Settings | §S-08 | `app/(tabs)/settings.tsx` | tab | settings/reading/sync/notif | all rows | reminderTime units (6) | connected | notif permission E2E on device | P0 |
| S-09 | Reader | §S-09 | `app/read/[passage].tsx` | push from Home/Read/Plans | `bibleService`,`readingStore`,`userDataStore` | loading/error/not-found | referenceParser units; manual device pass | connected | verse-scroll; push/replace; device pass | P0 |
| M-01 | Word Detail | §M-01 | `src/components/study/WordDetailModal.tsx` | reader word tap | strongs/lexicon | loading/empty | manual | needs-update | a11y label + `accessibilityViewIsModal` | P1 |
| M-02 | Strong's Search | §M-02 | `src/components/study/StrongsSearchModal.tsx` | word detail | strongsSearchService | loading/empty | manual | needs-update | a11y label | P1 |
| M-03 | Highlight menu | §M-03 | `src/components/study/HighlightMenu.tsx` | verse long-press | userDataStore | — | manual | needs-update | a11y + swipe-to-dismiss | P1 |
| M-04 | Note editor | §M-04 | `src/components/study/NoteEditor.tsx` | highlight menu | userDataStore | edit/new | manual | needs-update | a11y label | P1 |
